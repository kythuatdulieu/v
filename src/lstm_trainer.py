...
class LSTMTrainer:
    def __init__(self, config_name, random_seed=28112001):
        ...
        # ✅ FIX #3: Thêm target scaler
        # LSTM cần scale target để học tốt hơn (neural network nhạy cảm với scale)
    ...
    def load_data(self, data_folder):
        ...
        if len(y_train_raw.shape) > 1 and y_train_raw.shape[1] > 1:
            ...
            y_train_raw = y_train_raw[:, -1]  # Last day
            y_test_raw = y_test_raw[:, -1]    # Last day
        elif len(y_train_raw.shape) > 1:
            y_train_raw = y_train_raw.squeeze()
            y_test_raw = y_test_raw.squeeze()
        
        # ✅ FIX #3: Scale target (y) để LSTM học tốt hơn
        # Features đã được scale → Target cũng nên scale để cân bằng
        # Fit scaler trên training data, transform cả train và test
        ...
        # Lưu y gốc để đánh giá sau (trên thang đo gốc)
    ...
    def create_model(self, units, n_layers, dropout, input_shape):
        """
        Create LSTM model with regularization to prevent overfitting
        - Dropout: Applied to inputs and outputs
        - Recurrent Dropout: Applied to hidden states (prevents overfitting in temporal patterns)
        """
        model = Sequential()
        ...
        if n_layers == 1:
            model.add(LSTM(
                ...
                dropout=dropout,
                recurrent_dropout=0.2  # Regularize hidden states
            ))
        else:
            model.add(LSTM(
                units, 
        ...
        # Additional LSTM layers with recurrent dropout
    ...
    def evaluate(self):
        ...
        # Predictions (trên dữ liệu đã scale)
        ...
        # ✅ FIX #3: Inverse transform về thang đo gốc để đánh giá
        # Predictions và true values đều về thang đo gốc (mét)
        ...
        # Metrics trên thang đo GỐC (original scale - mét)
        train_metrics = {
